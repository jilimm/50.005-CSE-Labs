...
public class ProcessGraphNode implements Comparable<ProcessGraphNode>{
    ...
    //allows us to sort the nodes in numerical order according to nodeID

    @Override
    public int compareTo(ProcessGraphNode node){
        if (this.nodeId==node.getNodeId()){
            return 0;
        }
        else if (this.nodeId>node.getNodeId()){
            return 1;
        }else{
            return -1;
        }
