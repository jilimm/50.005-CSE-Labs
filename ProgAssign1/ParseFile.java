    ...
    public static void generateGraph(File inputFile) {
        try{
            ...
            while(fileIn.hasNext()){
                ...
                //setup input
                if (quatiles[2].equals("stdin")){
                    ProcessGraph.nodes.get(index).setInputFile(null);
                }else{
                    ProcessGraph.nodes.get(index).setInputFile(new File(quatiles[2]));
                }
                if (quatiles[3].equals("stdout")){
                     ProcessGraph.nodes.get(index).setOutputFile(null);
                }else{
            ...
            //setup parents for each node in the graph
            ...
            //mark initial nodes as runnable
            for (ProcessGraphNode node:ProcessGraph.nodes) {
